...
export class CallService {
    ...
    async create(call: CreateCallDto): Promise<{ id: string }> {
        ...
        this.logger.log(`Saving the file uuid: ${ uuid }, call: ${ JSON.stringify(call, null, 2) }`);
        const saveFileResult = await this.fileService.saveFile(uuid, call.audio_url);
        this.logger.log(`Save File Result: ${ JSON.stringify(saveFileResult, null, 2) }`);
